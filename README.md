...
При нажатии на кнопку `с`, программа переключается на получение изображения с IP-камеры. IP-адресс камеры задается в файле 'config.txt' в папке 'docs' в корневой директории проекта. Если этот файл не найден, то программа автоматически переключается на работу с камерой, подключенной к компьютеру.

При нажатии на клавишу `Enter`, выводятся все окна графики. При повторном нажатии, все окна закрываются.
...
## Sequence diagram
Примерная ди==аграмма работы программы
